...
class CGE():
    ...
    def __init__(self, SAM, psi1 = 1.12, psi2 = 0.5, psi3 = 0.601, 
                            sigma1   = 1.81, sigma2   = 0.8, sigma3   = 0.4):  
        self.SAM = SAM
        shm = self.SAM.loc
...
[   X1_base, L1_base, K1_base, I11_base, I21_base, I31_base, Ir1_base, Ib1_base, Z1_base, E1_base, D1_base, Q1_base, M1_base, 
            ...
            C1_base, C2_base, C3_base,  Cr_base, Cb_base, Y_base, Yd_base, 
            ...
            Sf_base, r_base ] = result1.x

print(Z1_base)


psilist   = np.linspace(0.5,2.5,10)
sigmalist = np.linspace(0.5,2.5,10)
...
Dlist = []
Mlist = []
resultdf = pd.DataFrame(columns = ["Z","E", "D", "M"], index = m1.init_values_str )
...
for psi in psilist:
    for sigma in sigmalist:
        ...
        m2 = CGE(SAM, psi1 = psi, sigma1 = sigma)
        ...
        [   X1_cfrun, L1_cfrun, K1_cfrun, I11_cfrun, I21_cfrun, I31_cfrun, Ir1_cfrun, Ib1_cfrun, Z1_cfrun, E1_cfrun, D1_cfrun, Q1_cfrun, M1_cfrun, 
            ...
            C1_cfrun, C2_cfrun, C3_cfrun,  Cr_cfrun, Cb_cfrun, Y_cfrun, Yd_cfrun, 
            ...
            Sf_cfrun, r_cfrun ] = result2.x
        
        # print(Z3_cfrun)
        
        Zlist.append((Z1_cfrun - Z1_base) / Z1_base *100)
        Elist.append((E1_cfrun - E1_base) / E1_base *100 )
        Dlist.append((D1_cfrun - D1_base) / D1_base *100)
        Mlist.append((M1_cfrun - M1_base) / M1_base *100)

       
X,Y = np.meshgrid(psilist, sigmalist)
Z = np.array(Zlist).reshape(len(psilist), len(sigmalist)).transpose()
E = np.array(Elist).reshape(len(psilist), len(sigmalist)).transpose()
D = np.array(Dlist).reshape(len(psilist), len(sigmalist)).transpose()
M = np.array(Mlist).reshape(len(psilist), len(sigmalist)).transpose()
...
surf = ax1.plot_surface(X, Y, Z, cmap = plt.cm.binary,  edgecolor ='black')
surf = ax2.plot_surface(X, Y, E, cmap = plt.cm.binary,  edgecolor ='black')
surf = ax3.plot_surface(X, Y, D, cmap = plt.cm.binary,  edgecolor ='black')
surf = ax4.plot_surface(X, Y, M, cmap = plt.cm.binary,  edgecolor ='black')

ax1.set_title("Panel A: Yurtiçi Üretim", fontweight = "bold", fontsize = 12)
ax1.set_xlabel('Dönüşüm Esnekliği', fontweight = "bold")
ax1.set_ylabel('İkame Esnekliği', fontweight = "bold")
ax1.set_zlabel('Miktar (% Değişim)', fontweight = "bold")

ax2.set_title("Panel B: İhracat", fontweight = "bold", fontsize = 12)
ax2.set_xlabel('Dönüşüm Esnekliği', fontweight = "bold")
ax2.set_ylabel('İkame Esnekliği', fontweight = "bold")
ax2.set_zlabel('Miktar (% Değişim)', fontweight = "bold")

ax3.set_title("Panel C: Yurtiçi Arz", fontweight = "bold", fontsize = 12)
ax3.set_xlabel('Dönüşüm Esnekliği', fontweight = "bold")
ax3.set_ylabel('İkame Esnekliği', fontweight = "bold")
ax3.set_zlabel('Miktar (% Değişim)', fontweight = "bold")

ax4.set_title("Panel D: İthalat", fontweight = "bold", fontsize = 12)
ax4.set_xlabel('Dönüşüm Esnekliği', fontweight = "bold")
ax4.set_ylabel('İkame Esnekliği', fontweight = "bold")
ax4.set_zlabel('Miktar (% Değişim)', fontweight = "bold")



resultdf.to_excel("Simulation2.xlsx")
plt.tight_layout()
plt.show()
